...
def filter_by_currency(get_transacts: List[Dict], currency: str = "USD") -> Iterator[list] | str:
    ...
    """
    iterator_for_transacts = (
        transact for transact in get_transacts if transact["operationAmount"]["currency"]["name"] == currency
    )
...
def transaction_descriptions(list_with_trans: List[Dict]) -> Iterator[str]:
    """Генераторная функция, которая принимает список словарей с транзакциями
...
def card_number_generator(start: int, end: int) -> Iterator[str]:
    ...
    if start < 1 or end > 9999999999999999 or start > end:
        raise (
            ValueError(
                """Диапазон должен быть от 1 до 9999999999999999 и начальное значение
            должно быть меньше или равно конечному."""
            )
        )
